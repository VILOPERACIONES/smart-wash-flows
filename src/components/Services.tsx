...
const Services: React.FC = () => {
  ...
  useEffect(() => {
    const fetchServicios = async () => {
      try {
        ...
        if (data.success) {
          // Filtrar solo activos y ordenar por el campo orden
          const serviciosActivos = data.data
            .filter((s) => s.activo)
  ...
  const renderAutoservicioCard = (servicio: Servicio) => (
    <article
      key={servicio.id}
      className="w-full max-w-[496px] border shadow-[0_1px_3px_0_rgba(0,0,0,0.12),0_1px_2px_0_rgba(0,0,0,0.24)] overflow-hidden bg-white rounded-3xl border-solid border-gray-200"
    >
      {servicio.mostrar_imagen && servicio.imagen && (
        <img
          src={servicio.imagen}
          alt={`Servicio de ${servicio.nombre} en A LAVAR`}
      ...
      <div className="p-10">
        <div className="flex items-center gap-4 mb-5">
          ...
          <div>
            ...
            <p className="text-[#003A9E] font-poppins text-sm font-normal leading-5">{servicio.subtitulo}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[#003A9E] text-sm font-poppins font-normal leading-5">
            Ciclo de Lavado y Secado ~{servicio.tiempo_desde || 80} {servicio.unidad_tiempo || "min"}
          </span>
        </div>
      </div>
    </article>
  ...
  const renderEncargadoCard = (servicio: Servicio) => (
    <article
      key={servicio.id}
      className="w-full max-w-[496px] shadow-[0_1px_3px_0_rgba(0,0,0,0.12),0_1px_2px_0_rgba(0,0,0,0.24)] overflow-hidden rounded-3xl bg-gradient-to-br from-[#1a4fd8] to-[#003A9E]"
    >
      <div className="relative">
        {servicio.mostrar_imagen && servicio.imagen ? (
          <img
            src={servicio.imagen}
            alt={`${servicio.nombre} del servicio de lavado por encargo`}
      ...
      <div className="p-10">
        ...
        <ul className="flex flex-col gap-4 mb-8">
          {servicio.caracteristicas.map((item, index) => (
            <li key={index} className="flex items-start gap-3">
              ...
              <span className="text-sm font-poppins font-normal leading-5 text-white">{item}</span>
            </li>
          ))}
        </ul>
        ...
        <div className="mb-5 p-4 py-10 rounded-2xl border-2 border-white border-opacity-20 bg-gradient-to-r from-blue-500/10 to-blue-600/10 flex flex-col items-center">
          ...
          <div className="text-center mb-4">
            <span className="text-white text-6xl font-poppins font-bold">${servicio.precio_desde}</span>
          ...
          {servicio.especial_min && servicio.especial_max && (
            <>
              ...
              <h2 className="font-poppins text-white xl:text-[14px] xl:leading-[20px] font-medium mb-4">
                {servicio.etiqueta || "Edredones y Hamacas"}
              </h2>
              <div className="text-center mb-4">
      ...
      <section id="servicios" className="w-full bg-white pt-[61px] pb-32 px-5">
        <div className="max-w-[1400px] mx-auto my-0">
          ...
          <div className="text-center py-10">
            <p className="text-[#003A9E] font-poppins text-lg">
              No pudimos cargar los servicios. Por favor, intenta de nuevo más tarde.
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-[1400px] mx-auto my-0">
        ...
        <div className="flex justify-center items-start gap-8 flex-wrap">
          {servicios.map((servicio) =>
            servicio.tipo === "autoservicio"
              ? renderAutoservicioCard(servicio)
              : renderEncargadoCard(servicio)
          )}
        </div>
